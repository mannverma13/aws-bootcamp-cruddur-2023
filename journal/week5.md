...
schema-load  scrpt use to create dynamo db and schema for the table.
...
def get_user_uuids():
  ...
  my_user    = next((item for item in users if item["handle"] == 'mannverma'), None)
  other_user = next((item for item in users if item["handle"] == 'manu'), None)
  results = {
    'my_user': my_user,
